...
- **Rosters** preserve historical `TeamRoster` rows. `isActive` marks the player's current team for that season, and a partial unique index permits at most one active team per player/season. “Add new player” only creates a new Player plus initial membership and must reject existing identity matches; the existing-player flow owns assignments, trades, and unassignments. Never delete or retarget historical memberships.
